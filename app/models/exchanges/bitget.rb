class Exchanges::Bitget < Exchange
  ...
  def symbols
    ...
    return Result::Success.new(cached) if cached.present?

    symbols_data = tickers.available.pluck(:base, :quote).map do |base, quote|
      ExchangeApi::Markets::MarketSymbol.new(base, quote)
    end
    Rails.cache.write(cache_key, symbols_data, expires_in: 1.hour)
  ...
  def parse_order_status(status)
    ...
    when 'full_fill'
      :closed
    when 'cancelled'
      Rails.logger.info("Order cancelled on #{name}: #{status}")
      :closed
    else
