class Transaction < ApplicationRecord
  ...
  before_create :set_exchange, if: -> { !bot.dca? }
  ...
  after_create_commit -> { bot.broadcast_new_order(self) if bot.dca? }
  after_update_commit -> { bot.broadcast_updated_order(self) if bot.dca? }
  after_commit lambda {
                 Bot::UpdateMetricsJob.perform_later(bot) if bot.dca? && custom_quote_amount_exec_changed?
               }, on: %i[create update]
  after_commit lambda {
